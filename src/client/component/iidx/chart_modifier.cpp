...
namespace iidx::chart_modifier
{
	...
	using chart_ro_t = std::span<iidx::event_t>;
	using chart_t = std::vector<iidx::event_t>;

	auto find_event_backward(const chart_ro_t& chart, iidx::event_type_t type, int8_t param, chart_ro_t::iterator start_from)
	{
		if (!chart.size())
			return chart.end();
	...
	auto find_event_forward(const chart_ro_t& chart, iidx::event_type_t type, int8_t param, chart_ro_t::iterator start_from)
	{
		if (!chart.size())
			return chart.end();
	...
	{
		chart_t output;
		iidx::event_t bpm_current = {};
		int last_scratch_back = 0;

		for (auto& ev : chart)
		{
			if (ev.type == iidx::tempo)
				bpm_current = ev;
		...
		std::stable_sort(output.begin(), output.end(), [](const iidx::event_t& a, const iidx::event_t& b)
			{
	...
	{
		chart_t output;
		std::array<iidx::event_t, 8> current_samples_p1{};
		std::array<iidx::event_t, 8> current_samples_p2{};
		...
		for (auto it = chart.begin(); it < chart.end(); it++)
		{
			...
			if (ev.type == iidx::sample_p1)
			{
				current_samples_p1[ev.param] = ev;
			}

			if (ev.type == iidx::sample_p2)
			{
			...
			{
				auto& sample = (ev.type == iidx::note_p1 ? current_samples_p1 : current_samples_p2)[ev.param];
				if (scratch_sample != sample.value)
				{
					output.push_back(iidx::event_t
						{
							.tick = scratch_tick + (ev.tick - scratch_tick) / 2,
							.type = static_cast<iidx::event_type_t>(ev.type + 2),
							.param = 7,
							.value = sample.value,
						});
				}
	...
	chart_t all_charge_note_modifier(chart_ro_t& chart)
	{
		...
		for (auto i = chart.begin(); i < chart.end(); i++)
		{
			...
			{
				auto next = std::find_if(i + 1, chart.end(), [i](const iidx::event_t& e)
					{
				...
				{
					auto sample = find_event_backward(chart, static_cast<iidx::event_type_t>(next->type + 2), next->param, next);
	...
	chart_t fixed_random_modifier(chart_ro_t& chart)
	{
		...
		for (auto& ev : chart)
		{
			if (ev.type < iidx::tempo && ev.param != 7)
	...
	void post_load_chart(iidx::event_t* raw_chart, int unk)
	{
		if (modifier_flag == modifier_t::none)
			return post_load_chart_hook.invoke<void>(raw_chart, unk);

		auto* eos = std::find_if(raw_chart, raw_chart + MAX_EVENT, [](const iidx::event_t& e) {
			return e.type == iidx::end_of_song;
			});
		...
		std::memcpy(raw_chart, chart.data(), chart.size() * sizeof(iidx::event_t));

		return post_load_chart_hook.invoke<void>(raw_chart, unk);
	}

	utils::hook::detour get_option_str_hook;
	void get_option_str(DWORD* a1, iidx::string* option_str, int a3, __int64 a4)
	{
